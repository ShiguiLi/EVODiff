...
class NoiseScheduleEDM:
    ...
    def marginal_kappa(self, t):
        """
        Compute kappa_t = sigma_t / alpha_t of a given continuous-time label t in [0, T].
        """
        # return torch.exp(-self.marginal_lambda(t))
        return t.to(torch.float64)

    def inverse_kappa(self, kappa):
        """
        Compute the continuous-time label t in [0, T] of a given kappa_t.
        """
        # return self.inverse_lambda(-torch.log(kappa)) 
        return t.to(torch.float64) 
